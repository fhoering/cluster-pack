...
def get_conda_env_name(conda_env_path=None, reqs: List[str] = None, env_id=None):
...
def create_and_pack_conda_env(reqs: List[str] = None, output: str = None) -> str:
    project_env_name = get_conda_env_name(reqs=reqs)

    _logger.info(f"Found project env name {project_env_name}")
    env_path = get_or_create_conda_env(project_env_name)
